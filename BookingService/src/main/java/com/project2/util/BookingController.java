...
public class BookingController {
	...
	public List<Booking> findBookingsByHotel(@PathVariable Long hotelId) {
		...
		return service.findAll().stream().filter(x -> x.getHotelId()==hotelId).collect(Collectors.toList());
	}
	...
	public List<Booking> findBookingsByCustomer(@PathVariable Integer userId){
		...
		return service.findAll().stream().filter(x -> x.getUserId()==userId).collect(Collectors.toList());
	}
}
